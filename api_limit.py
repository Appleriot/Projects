...
window.title("Doakes Pokedex")
window.config(padx=10, pady=10)

title_label = tk.Label(window, text='Doakes Pokedex')
...
def load_pokemon():
    ...
    respone = http.request('GET', pokemon.sprites.front.get('default'))
    image = PIL.Image.open(BytesIO(respone.data))
...
label_id = tk.Label(window, text='Id or Name')
label_id.config(font=('Arial', 20))
